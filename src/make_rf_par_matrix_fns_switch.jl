    ...
    for i in eachindex(tip_states)
       ...
       # full_tip_state_group=state_groups[in.(tip_states[i],state_groups)][1]
        ...
        if forbidden_fund!==NaN
            ...
            if(size(forbidden_fund)[2]>1)

                for col in 1:length(size(forbidden_fund))
...
function make_rf_par_matrix(nbiomes::Int64,
    ...
    for from in eachindex(biome_range_states) for to in eachindex(biome_range_states) 
        ...
        #check to make sure there is only a shift in a single biome, most moves aside from switches have only a single biome change
        ...
        #check for moves with only 2 biomes changing (for switches)
        elseif (sum(no_shifts) == (nbiomes-2))


          #check to see if a single gain is present in one biome and a single loss is present in aother
          switch        = (sum(single_gains) == 1) && (sum(single_losses) ==1)
          if switch && allow_real_switches
            #then check if before single gain a fundamental affinity is present
            gained_from_f = (biome_range_states[from][biome_diff.!=0 .&& single_gains] .==1)[1]
            if gained_from_f
                #then check if single loss is to a fundamental affinity
                lost_from_r   = (biome_range_states[to][biome_diff.!=0 .&& single_losses] .==1)[1]
                if lost_from_r
                    par_matrix_string[from, to]=""

                    if by_rf==true
                        par_matrix_string[from, to]= par_matrix_string[from, to]* "_rf" *  string(1,2)
                    end
                    ...
                    if by_gainloss==true
                        par_matrix_string[from, to]= par_matrix_string[from, to] * "_sw" 
                    end
...
function make_rf_par_matrix(nbiomes::Int64,
                            max_real_biome::Int64,
                            allow_real_switches::Bool =true,
                            allow_realfun_switches::Bool =false,
                            allow_double_gains::Bool  =true,
                            allow_double_losses::Bool =false,
                            allow_single_gains::Bool  =true,
                            allow_single_losses::Bool =true,
    ...
    for from in eachindex(biome_range_states) for to in eachindex(biome_range_states) 
        ...
        elseif (sum(no_shifts) == (nbiomes-2))
          ...
          #check to see if a single gain is present in one biome and a single loss is present in aother
          real_switch        = (sum(single_gains) == 1) && (sum(single_losses) ==1)
          realfun_switch     = (sum(double_gains) == 1) && (sum(double_losses) ==1)

          if real_switch && allow_real_switches
            #then check if before single gain a fundamental affinity is present
            gained_from_f = (biome_range_states[from][biome_diff.!=0 .&& single_gains] .==1)[1]
            if gained_from_f
                #then check if single loss is to a fundamental affinity
        ...
        if  realfun_switch && allow_realfun_switches
            #then check if before double gain a fundamental affinity is present
            gained_from_f = (biome_range_states[from][biome_diff.!=0 .&& double_gains] .== 0)[1]
            if gained_from_f
                #then check if single loss is to a fundamental affinity
                lost_from_r   = (biome_range_states[to][biome_diff.!=0 .&& double_losses] .==0)[1]
                if lost_from_r
                    par_matrix_string[from, to]=""
