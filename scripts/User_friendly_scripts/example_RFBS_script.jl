...
#include(joinpath(pwd(), "src", "User_friendly_fn_wrappers.jl"))
...
include(joinpath(pwd(), "src", "rf_mcmc_fast.jl"))
include(joinpath(pwd(), "src", "User_friendly_fn_wrappers.jl"))

include(joinpath(pwd(), "src", "User_friendly_fast_wrappers.jl"))
...
tip_probs, nbiomes, rf_states = read_data_csv(fn; adj_matrix=fund_adj_matrix, max_real_biome=3)


for i in 1:length(tip_probs)

    print(tree.tlab[i])

    display(hcat((rf_states[j] for j in findall(tip_probs[i] .== 1.0))...))     #[print(st2rfDict[i]) for i in findall(tip_probs[i] .== 1.0)]
