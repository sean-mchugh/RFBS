...
Mny of these are admittedly not very user-friendly, RFBS originally accepted multiple data files for "standard" observed affintiies, excluded and included enabled affinities. To make RFBS more user frinedly and generally-applicable, I have included a set of wrapper functions and an example script to "plug and play". The user only needs to provide the phylogeny and a single affinity data file – a csv where the first column is species names that match the phylogenetic tip labels, and the subsequent columns score biome affinities (2=Established Affinity, 1=an Explicitly Enabled Affinity, 0=and Explicitly Non-Affinity, and NA= Unobserved, Either Enabled Affinity or a Non-Affinity). An additional script is provided for users to generate ancestral affinitiy reconstruction figures as we did in the paper. MCMC chains can also be viewed using the program Tracer (downloaded here https://github.com/beast-dev/tracer/releases/tag/v1.7.2). 
